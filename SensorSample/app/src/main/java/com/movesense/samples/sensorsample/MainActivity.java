...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemLongClickListener, AdapterView.OnItemClickListener  {
    ...
    //user state: 0 comfortable, 1 scared
    ...
    void updateValues(float x, float y, float z){
        prevX=x;
        prevY=y;
        prevZ=z;
    }

    void updateDifference(float x, float y, float z){
        differenceX = x-prevX;
        differenceY = y-prevY;
        differenceZ = z-prevZ;
    }
                ...
                strContract, new MdsNotificationListener() {
                    ...
                    public void onNotification(String data) {
                        ...
                        if (accResponse != null && accResponse.body.array.length > 0) {
                            String accStr;
                            //String accStr =
                              //      String.format("%.02f, %.02f, %.02f \n %.02f, %.02f, %.02f ", accResponse.body.array[0].x, accResponse.body.array[0].y, accResponse.body.array[0].z,prevX,prevY,prevZ);
                            if(differenceX>0.4 || differenceY>0.4 || differenceZ>0.4){
//                                accStr =
  //                                      String.format("%.02f, %.02f, %.02f", differenceX,differenceY,differenceZ);
                            ...
                            updateDifference(((float) accResponse.body.array[0].x),((float) accResponse.body.array[0].y),((float) accResponse.body.array[0].z));
